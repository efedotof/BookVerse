...
import 'package:magazine/screens/auth/until/addition%20unit/UserModel.dart';
import 'package:magazine/widgets/additional%20tools/cardsing.dart';
import 'package:magazine/widgets/additional%20tools/list_data_text.dart';
import 'addition unit/additionalСontinuationWidget.dart';
    ...
    return Scaffold(
      body: Stack(
        ...
        SingleChildScrollView(
                                  ...
                                  child: CircleAvatar(
                                    ...
                                    child: StreamBuilder(
                                      ...
                                      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
                                        if(!snapshot.hasData){
                                          return (const Center(child: Text('Здесь ничего нет'),));
                                        }else{
                                          return ClipOval(child: Image.network(snapshot.data!.docs[snapshot.data!.docs.length - 1]['URL'],height: 140,width: 140, fit: BoxFit.cover, ));
                                        }
                                      } ,
                                    ),
          ...
          spacing: 16.0, 
          children: [
            for (int index = 0; index < book_as_famali.length; index++)
              SizedBox(
          width: MediaQuery.of(context).size.width *0.4, // Ширина обертки - половина ширины экрана
          height: MediaQuery.of(context).size.height * 0.4 - 10,
          child: cards(
            books: book_as_famali,
            indexs: index,
            image_netvorsk: "assets/testImage.jpg",
          ),
              ),
          ],
                )
